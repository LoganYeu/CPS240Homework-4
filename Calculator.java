...
public class Calculator extends GUI {
            ...
            b.pressedProperty().addListener((observable, wasPressed, pressed) -> {
                ...
                if (pressed) {
                    ...
                    if (b.getText().equals("=")) {
                        ...
                        // Only entering an operator such as x or + will yield this error after pressing (=). Example (+=). Also catches errors from user entering more than one operation Such as (5+4-6). Lastly, catches an error that occurs if user enters one side of operation (2+)
        ...
        for(Button b : functionButtons){
            b.pressedProperty().addListener((observable, wasPressed, pressed) -> {
                ...
                if (pressed) {
                    ...
                    // Give functionality for "DEL" button which deletes the most recent character entered by the user.
                    else if (b.getText().equals("DEL")) {
                        System.out.println(b.getText());
                        buttonPress(b);
                        String[] currentOperationText = operation.getText().split("");
                        // New array with the removed last element of string to effectively delete the last thing inputted.
    ...
    public void equals(String currentOperation) {
        ...
        // Variables and for loop below ensure that if there are an illegal amount of decimals a error is thrown. Does so by ensuring there are no more than 2 "." in one side of the operation. Example being no more than 1 period in a problem such as (3.4+5.3 wouldn't throw an error, 3.4.3+5.3 would throw the error)
        String previousElementOfArray = "";
        // Loops through each part of the operation.
        for (String s : parts) {
            // Throws error if only a "." is on either side of an operation. Example: (3+.)
            if (s.equals(".")) {
                throw new IllegalArgumentException();
            }
            // Splits each part of the operation further on "." to count how many "." there are in one side of an operation.
            String [] tempArray = s.split("(?<=[\\.])|(?=[\\.])");
            int decimalsInCurrentPart = 0;

            // Loops through the array to see if there is more than one "." on one side of a operation and throws error. Example: (3..3+5) and (3.3.4+5).
            for (int i = 0; i < tempArray.length; i++) {
                if (tempArray[i].equals(".")) {
                    decimalsInCurrentPart += 1;
                }

                if (decimalsInCurrentPart >= 2) {
                    throw new IllegalArgumentException();
                }
            }
        }
